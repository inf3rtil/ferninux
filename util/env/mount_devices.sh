...
mount_devices(){
    echo "EXEC: mount devices -------------------------------------------------"
    disk_loop=$(mount | grep $ROOTFS | cut -d ' ' -f1)
	...
	if test -f "$BUILD_DIR/$VDISK_FILENAME"; then
		...
		mount -v $disk_loop$VDISK_EFI_PART $ROOTFS/boot/efi
	    fi
